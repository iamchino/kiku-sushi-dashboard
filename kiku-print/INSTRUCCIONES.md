# 🖨 KIKU Print — puente de impresión propio
...
veces no". KIKU Print dibuja el QR como **imagen** (píxeles): cualquier
...
**Se compila y actualiza solo**: cada push a este repo que toque `kiku-print/`
...
→ link **"Descargar KIKU Print"** (o directo:
`https://kiku-sushi-dashboard.vercel.app/descargas/KikuPrint.exe`).
Guardalo en una carpeta fija, por ejemplo `C:\KikuPrint`.

### 2. Primer arranque
Doble click en `KikuPrint.exe`:
...
"KIKU Print funcionando" **con candado y sin advertencia**, está listo.
...
directo** → `C:\KikuPrint\KikuPrint.exe` → Finalizar.
...
| Cambió la IP de la PC | Cerrá y abrí KikuPrint.exe (regenera el certificado del servidor; el `.crt` instalado sigue valiendo). Actualizá la IP en el dashboard. Ideal: IP fija desde el router. |
...
- El código vive en `kiku-print/` del repo del dashboard. Cualquier push que
  lo toque dispara `.github/workflows/kiku-print.yml`: compila, y commitea el
...
- **Importante**: para que los equipos se actualicen hay que subir el número
  ...
  versión contra `kiku-print-version.json`.
