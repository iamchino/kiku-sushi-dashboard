-- ############################################################################
--
--  KIKU SUSHI — SIMULACRO de la fase 4
--
--  Simula cada rol contra cada tabla, aplica los cambios, vuelve a simular,
--  te muestra el diff y hace ROLLBACK.
--
--  *** NO APLICA NADA. La base queda exactamente como estaba. ***
--  Se puede correr en producción sin riesgo.
--
--  Cómo leer el resultado:
--    (+) gana    → ese rol pasa a poder acceder. Es la matriz haciendo efecto.
--    (-) PIERDE  → deja de poder. REVISALO UNO POR UNO: si una pantalla que
--                  ese rol usa depende de esa tabla, se va a romper.
--
--  Si el resultado viene vacío, la fase 4 no le cambia el acceso a nadie.
--  Cuando el diff sea el esperado, aplicá SQL_EDITOR_FASE4.sql.
--
-- ############################################################################
...
-- ─── Utilería: medir el acceso real de un rol a una tabla ───────────────────
-- No alcanza con leer el texto de las policies: hay que EVALUARLAS. Seteamos
-- request.jwt.claims (que es de donde auth.jwt() saca los datos) y ejecutamos
-- la expresión de cada policy aplicable.
--
-- Por qué no simplemente hacer `select from la_tabla`: RLS no tira error
-- cuando bloquea, devuelve cero filas. Con una tabla vacía, "bloqueado" y
-- "permitido" se ven igual. Evaluar la expresión es determinista.
create or replace function pg_temp.acceso(p_rol text, p_tabla text, p_cmd text)
returns boolean
language plpgsql
as $fn$
declare
...
begin
  perform set_config('request.jwt.claims',
    json_build_object(
      ...
      'role',  'authenticated',
      'email', 'simulacro@kikusushi.com.ar',
      'app_metadata', json_build_object('role', p_rol)
    )::text, true);
  ...
  for pol in
    ...
    where schemaname = 'public' and tablename = p_tabla
      and (cmd = p_cmd or cmd = 'ALL')
      and ('authenticated' = any(roles) or 'public' = any(roles))
  loop
    expr := coalesce(pol.qual, pol.with_check, 'true');
    -- Las que dependen de la fila (auth.uid() contra una columna) no se pueden
    -- evaluar sueltas. Son las de acceso propio, que esta fase no toca.
    if expr ~ 'auth\.uid\(\)' then continue; end if;
    begin
      execute 'select (' || expr || ')' into res;
      if coalesce(res, false) then return true; end if;
...
select r.id as rol, x.tabla,
       pg_temp.acceso(r.id, x.tabla, 'SELECT') as lee,
       pg_temp.acceso(r.id, x.tabla, 'UPDATE') as escribe
from public.roles r
cross join (select distinct tabla from public.recurso_tablas) x
where to_regclass('public.' || quote_ident(x.tabla)) is not null;

-- ─── Aplicar la fase 4 (se revierte al final) ───────────────────────────────
-- ============================================================================
...
-- ─── Foto DESPUÉS ───────────────────────────────────────────────────────────
create temporary table _despues on commit drop as
select r.id as rol, x.tabla,
       pg_temp.acceso(r.id, x.tabla, 'SELECT') as lee,
       pg_temp.acceso(r.id, x.tabla, 'UPDATE') as escribe
from public.roles r
...
select
  case
    when a.lee and not d.lee          then '(-) PIERDE lectura'
    when not a.lee and d.lee          then '(+) gana lectura'
    when a.escribe and not d.escribe  then '(-) pierde escritura'
    else '(+) gana escritura'
  end as cambio,
  a.rol,
  a.tabla,
  format('lee=%s escribe=%s', a.lee, a.escribe) as antes,
  format('lee=%s escribe=%s', d.lee, d.escribe) as ahora
from _antes a
join _despues d on d.rol = a.rol and d.tabla = a.tabla
where (a.lee, a.escribe) is distinct from (d.lee, d.escribe)
order by case when a.lee and not d.lee then 0 else 1 end, a.rol, a.tabla;

rollback;

-- ############################################################################
--  Terminó con ROLLBACK. Nada se aplicó.
-- ############################################################################
