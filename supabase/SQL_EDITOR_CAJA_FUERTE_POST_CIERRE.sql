-- ════════════════════════════════════════════════════════════════════════════
-- CAJA FUERTE: RETIRO POST-CIERRE — pegar completo en el SQL Editor
--
-- Arregla el circuito del efectivo: ahora el retiro a caja fuerte funciona
-- TAMBIÉN con la caja cerrada (sale del efectivo del último cierre) y la
-- próxima apertura deja de arrastrar esa plata. Antes solo funcionaba con
-- turno abierto y el "ajuste" no descontaba nada del arrastre.
-- ════════════════════════════════════════════════════════════════════════════
...
-- Verificación: la función quedó con la lógica nueva (debe decir 'post_cierre')
select case when prosrc like '%post_cierre%' then '✔ retiro post-cierre instalado'
            else '✘ ALGO FALLÓ: la función no tiene la lógica nueva' end as estado
from pg_proc where proname = 'retirar_a_caja_fuerte';
