...
function PrecioCell({ item, onSave }) {
  ...
  return (
    ...
    >
      {precio > 0 ? `$${precio.toLocaleString('es-AR', { maximumFractionDigits: 2 })}` : '-'}
    </button>
  )
}
...
function ItemRow({ item, showCostColumns = true, updatePrecio, openEdit, setDeleteTarget }) {
  ...
  return (
    <>
      ...
      >
        <td className="pl-3 pr-1 py-3">
          <button
            ...
            disabled={movs.length === 0 && !item.notas}
        ...
        <td className="px-3 py-3 hidden md:table-cell">
          <span className="text-xs" style={{ color: 'var(--text-muted)' }}>
            {isProduccion ? (item._receta?.nombre || '-') : (item.proveedor || '-')}
          </span>
        </td>
        <td className="px-3 py-3 hidden md:table-cell">
      ...
      {showHistory && (movs.length > 0 || item.notas) && (
        <tr>
          <td colSpan={showCostColumns ? 9 : 7} style={{ padding: 0, background: 'var(--bg-input)' }}>
                ...
                <div>
                  ...
                  <div className="max-h-40 overflow-y-auto space-y-0">
                    {movs.slice(0, 20).map(m => {
                      const cfg = TIPO_MOV[m.tipo] || { label: m.tipo, color: '#a1a1aa', icon: 'i' }
                      const fecha = new Date(m.created_at)
                      return (
                        <div key={m.id} className="flex items-center gap-3 py-1.5 text-xs" style={{ borderBottom: '1px solid var(--border)' }}>
    ...
    >
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 gap-3">
          <Package size={30} style={{ color: 'var(--text-xmuted)' }} />
                ...
                {(showCostColumns
                  ...
                  <th
                    ...
                    className={`px-3 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-left whitespace-nowrap
                      ...
                      ${showCostColumns && i === 6 ? 'hidden md:table-cell' : ''}
                      ${showCostColumns && i === 7 ? 'hidden lg:table-cell' : ''}
                    `}
                    style={{ color: 'var(--text-xmuted)' }}
                  >
                    {h}
...
export default function StockPage() {
  ...
  const [defaultTipoStock, setDefaultTipoStock] = useState('materia_prima')
  ...
  const filtered = useMemo(() => {
    ...
    if (estadoFiltro === 'criticos') list = list.filter(i => ESTADO_STOCK(i) === 'critico')
    if (estadoFiltro === 'bajos') list = list.filter(i => ESTADO_STOCK(i) === 'bajo')
    if (estadoFiltro === 'ok') list = list.filter(i => ['ok', 'medio'].includes(ESTADO_STOCK(i)))
    if (search.trim()) {
      const q = normalizeSearch(search)
      list = list.filter(i =>
        normalizeSearch(i.nombre).includes(q) ||
        normalizeSearch(i.proveedor).includes(q) ||
        normalizeSearch(i._receta?.nombre).includes(q)
      )
    }

    return [...list].sort((a, b) => {
      if (ordenFilas === 'az') return a.nombre.localeCompare(b.nombre)
      if (ordenFilas === 'reciente') {
        const dateA = new Date(a.stock_movimientos?.[0]?.created_at || a.created_at || 0).getTime()
        const dateB = new Date(b.stock_movimientos?.[0]?.created_at || b.created_at || 0).getTime()
        return dateB - dateA
      }
  ...
  const openEdit = (item) => {
    setModalEdit(item)
    setDefaultTipoStock(getTipoStock(item))
    setModoEdicion(true)
  }

  const openNew = (tipoStock = 'materia_prima') => {
    setModalEdit(null)
    setDefaultTipoStock(tipoStock)
    setModoEdicion(true)
  }
    ...
    <div className="p-4 md:p-6 space-y-5 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <button
            key={s.id}
            onClick={() => setEstadoFiltro(estadoFiltro === s.id ? 'todos' : s.id)}
            className="flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all"
            style={{
              background: estadoFiltro === s.id ? `${s.color}12` : 'var(--bg-card)',
              border: `1px solid ${estadoFiltro === s.id ? s.color + '55' : 'var(--border-card)'}`,
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <div className="relative">
          ...
          >
            ...
            <option value="az">Orden: Alfabetico</option>
            <option value="reciente">Orden: Mas recientes</option>
          </select>
          <ChevronDown size={12} className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: 'var(--text-xmuted)' }} />
        </div>
          ...
          <div className="relative w-full max-w-sm rounded-2xl p-6 space-y-4" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: '0 32px 64px rgba(0,0,0,0.3)' }}>
            <div className="text-center space-y-2">
              ...
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                "<span style={{ color: 'var(--text-primary)' }}>{deleteTarget.nombre}</span>" y su historial se eliminaran.
              </p>
            </div>
