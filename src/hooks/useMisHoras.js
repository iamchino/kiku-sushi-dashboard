...
import { rangoSemana, minutosExactos } from '../lib/horas'
...
//  - minutos: total de la semana en minutos exactos (sin bloques ni redondeo)
...
export function useMisHoras(refDate) {
  ...
  // Total de la semana: la suma de los minutos REALES de cada jornada cerrada.
  // Sin bloques ni redondeo — igual que la base.
  const minutos = useMemo(() => {
    let total = 0
    for (const j of jornadas) {
      if (!j.salida) continue
      total += minutosExactos(j.minutos_reales ?? j.minutos ?? 0)
    }
    return total
  }, [jornadas])
